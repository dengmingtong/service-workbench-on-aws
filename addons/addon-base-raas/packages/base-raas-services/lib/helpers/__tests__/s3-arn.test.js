...
describe('s3 arn helpers', () => {
  ...
  const arn9 = 'arn:aws:s3:::bucket-data';
  describe('parseS3Arn', () => {
    ...
    it('it returns correct the parts', () => {
      ...
      expect(parseS3Arn(arn9)).toStrictEqual(output('aws', 'bucket-data', '/'));
    });
  });
  ...
  describe('normalizeS3Arn', () => {
    it('Should return correct arn with a forward slash at end', () => {
      ...
      expect(normalizeS3Arn(arn9)).toEqual('arn:aws:s3:::bucket-data/');
    });
  });
  ...
  describe('normalizeBucketArn', () => {
    it('Should return correct arn without folder', () => {
      ...
      expect(normalizeBucketArn(arn9)).toEqual('arn:aws:s3:::bucket-data');
    });
  });
  ...
  describe('toS3Arn', () => {
    it('Should return correct S3 arn', () => {
      ...
      expect(
        toS3Arn({ bucket: 'bucket-data', awsPartition: 'aws', folder: 'users/example@example.com/my-study-1/' }),
      ).toEqual('arn:aws:s3:::bucket-data/users/example@example.com/my-study-1/');
    });
  });
});
